...
repeat_statement = repeat_statement = "repeat" block_statement "until" "(" relational_expression ")";
statement = block_statement | if_statement | while_statement | repeat_statement | function_statement | return_statement | print_statement | assignment | expression;
program = statement
"""
...
def parse_arithmetic_term(tokens):
    ...
    # MODULUS OPEARTOR ADDITIONS ==============================================

    while tokens[0]["tag"] in ["*", "/", "%"]: # added '%' to list of operators
...
# REPEAT-UNTIL LOOP ADDITIONS =================================================

# function to parse repeat-until loops
def parse_repeat_statement(tokens): 
    """
    repeat_statement = repeat_statement = "repeat" block_statement "until" "(" relational_expression ")";
    """
    # check for repeat token 
    assert tokens[0]["tag"] == "repeat" 

    # parse block statement
    block_statement, tokens = parse_block_statement(tokens[1:]) 

    # check for and consume 'until' token
    if tokens[0]["tag"] != "until":
        raise Exception(f"Expected 'until': {tokens[0]}")
    tokens = tokens[1:]

    # check for and consume '(' token
    if tokens[0]["tag"] != "(":
        raise Exception(f"Expected '(': {tokens[0]}")
    ...
    # return repeat-until AST and remaining tokens
    return {"tag": "repeat", "condition": condition, "do": block_statement}, tokens


def test_parse_repeat_statement():
    """
    repeat_statement = repeat_statement = "repeat" block_statement "until" "(" relational_expression ")";
    """ 
    ast = parse_repeat_statement(t("repeat {x=2;} until (5)"))[0]
    assert ast == {
        "tag": "repeat",
        "condition": {"tag": "<number>", "value": 5},
        "do": {
            "tag": "block",
            "statement": {
                "tag": "=",
...
def parse_statement(tokens):
    ...
    # check for possibility of repeat-until statement
    if tag == "repeat": 
        return parse_repeat_statement(tokens)
...
if __name__ == "__main__":
    for f in [
        ...
        # REPEAT UNTIL ADDITIONS ==============================================
        test_parse_repeat_statement,
        # =====================================================================
